...
## トラブルシューティング
* 起動後にログインエラーが表示される場合は公式ランチャーを起動してゲームのアップデートがないか確認してください。
* その他問題が発生した場合は、ツールと同じフォルダにある `.log` ファイルを確認してください。エラーの詳細が記録されています。
　※まずはconfig.jsonの削除、公式ランチャーでの通常起動を行った後、初回起動からやり直してください。
